...
!$omp parallel
    ...
    if (thread_id == 0) then
       allocate(fission_bank(6*work))
    else
       allocate(fission_bank(6*work/n_threads))
    end if
!$omp end parallel
    allocate(master_fission_bank(6*work), STAT=alloc_err)
#else
    allocate(fission_bank(6*work), STAT=alloc_err)
#endif
